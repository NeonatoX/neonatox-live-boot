...
| `62-embedded-etc` | pre-pack | embedded | skeleton `/etc` mínimo vendorizado de `initramfs/etc-embed/` (shells, os-release, lsb-release, resolv.conf, profile LFS); gana al 60; **elimina** `passwd`/`group` horneados del hook 20 (los regenera `setup_ssh()` en runtime) |
